...
function Income() {
  ...
  useEffect(() => {
    localStorage.setItem("activities", JSON.stringify(outcome));
  }, [outcome]);

  const getNextId = () => {
    if (outcome.length === 0) return 1;
    const maxId = Math.max(...outcome.map((activity) => activity.id));
    return maxId + 1;
  };

  const handleAdd = (newOutcomes) => {
    const updatedOutcome = [...outcome, { ...newOutcomes, id: getNextId() }];
    setOutcome(updatedOutcome);
    setOpen({ ...open, add: false });
  };

  const handleEdit = (updatedOutcome) => {
    setOutcome(
      outcome.map((outcomes) =>
        outcomes.id === updatedOutcome.id ? updatedOutcome : outcome
      )
    );
  ...
  const handleDelete = (id) => {
    setOutcome(outcome.filter((outcomes) => outcomes.id !== id));
